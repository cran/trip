...
tripGrid <- function(x, grid = NULL, method = "count", dur = NULL, ...) {
	...
	trip.list <- split(x[, x@TOR.columns], x[[x@TOR.columns[2]]])
	tor <- x@TOR.columns
	cnt <- 0
...
function (obj, cells.dim =c(100, 100),
    xlim =NULL, ylim = NULL, buffer = 0.1, cellsize = NULL)
{
   if ((is.null(xlim) | is.null(ylim)) & missing(obj)) stop("require at least a Spatial object, or xlim and ylim")
   ...
   if (!is.null(cellsize)) {
   	   ...
   	   if (!proj) {
		    cellsize <- c(cellsize[1]/(cos((pi/180) * mean(ylim)) * 1.852 * 60), 
		    cellsize[2]/(1.852 * 60))
	...
	} else   cellsize <- c(diff(xlim), diff(ylim))/cells.dim
    ...
    if (buffer > 0) {
        ...
        cellsize <- c(diff(xlim), diff(ylim))/cells.dim
    }
